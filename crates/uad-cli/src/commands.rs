...
pub async fn list_devices() -> Result<(), Box<dyn std::error::Error>> {
...
impl PackageListContext {
    ...
    pub fn display_config(&self) -> DisplayConfig {
        DisplayConfig {
            show_state: self.state_filter.map_or(true, |f| !f.is_specific()),
            show_removal: self.removal_filter.map_or(true, |f| !f.is_specific()),
        }
    }
}
...
pub async fn list_packages(
    ...
    let target_device = get_target_device(device).await?;
...
pub async fn change_package_state(
    packages: Vec<String>,
    device: Option<String>,
    user_id: Option<u16>,
    dry_run: bool,
...
pub async fn execute_with_fallback(
    ...
    // Execute commands
    for cmd in commands {
        match run_adb_shell_action(&device.adb_id, cmd.clone()).await {
...
pub async fn show_package_info(
    ...
    if let Some(device_id) = device {
        let target_device = get_target_device(Some(device_id)).await?;
...
pub async fn update_lists() -> Result<(), Box<dyn std::error::Error>> {
