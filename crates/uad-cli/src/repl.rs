...
async fn handle_repl_line(
    ...
    match parts[0] {
        ...
        "list" | "ls" => {
            handle_list_command(&parts[1..], device, user_id, uad_lists).await?;
        }
        "info" => {
            handle_info_command(&parts[1..], device, uad_lists).await?;
        }
        "uninstall" | "rm" => {
...
async fn handle_info_command(
    ...
    let state = get_package_state(&device.adb_id, package, None)?;
    println!("  State:       {}", state);
...
async fn handle_state_change_command(
    args: &[&str],
    device: &Phone,
    user: User,
...
async fn process_package_change(
    pkg_name: &str,
    device: &Phone,
    user: User,
    target_state: PackageState,
