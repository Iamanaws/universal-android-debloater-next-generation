...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::List {
            ...
            commands::list_packages(device, state, removal, list, search, user).await?;
        }
        Commands::Uninstall {
            packages,
            device,
        ...
        Commands::Info { package, device } => {
            commands::show_package_info(package, device).await?;
        }
        Commands::Update => {
            commands::update_lists().await?;
        }
        Commands::Completions { shell } => {
            commands::generate_completions(shell);
        }
        Commands::Repl { device, user } => {
            repl::repl_mode(device, user).await?;
        }
    }
